...
Future<Code> generateEnumFromJson(
    ClassTypeBuilder builder, String enumName, List<String> variantNames) async {
  final map = await builder.resolveIdentifier(_dartCore, 'Map');
  final string = await builder.resolveIdentifier(_dartCore, 'String');
  final object = await builder.resolveIdentifier(_dartCore, 'Object');
...
Future<Code> generateVariantFromJson(
    ClassTypeBuilder builder, String variantName, List<(String type, String name)> fields) async {
  final map = await builder.resolveIdentifier(_dartCore, 'Map');
  final string = await builder.resolveIdentifier(_dartCore, 'String');
  final object = await builder.resolveIdentifier(_dartCore, 'Object');
  final List<RawCode> parts = [];
  ...
  parts.add(RawCode.fromParts(
      ["final ${variantName}_ = json[r'$variantName'] as ", map, "<", string, ",", object, "?>;"]));
  for (final field in fields) {
    ...
    switch (fieldType) {
      ...
      default:
        ...
        parts.add(RawCode.fromParts([
          "final $fieldName = $fieldType.fromJson(${variantName}_[r'$fieldName'] as ",
          map,
          "<",
          string,
          ",",
